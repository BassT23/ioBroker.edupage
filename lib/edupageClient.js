...
class EdupageClient {
  ...
  // ===== Timetable data: currentttGetData =====
  // POST /timetable/server/currentttjs?_func=currentttGetData
  // payload: { args: [...], _gsh: "...." }
  async currentttGetData({ args, gsh }) {
    if (!Array.isArray(args)) throw new Error('currentttGetData: args must be an array');
    if (!gsh) throw new Error('currentttGetData: missing gsh');
