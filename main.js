...
class Edupage extends utils.Adapter {
  ...
  async onReady() {
    ...
    this.client = new EdupageClient({ http: this.http, baseUrl, log: this.log });
  ...
  async syncOnce() {
    try {
      ...
      // 1) Login (wenn noch nicht eingeloggt / Session weg)
      if (!this.isLoggedIn) {
        const tokRes = await this.client.getToken({
          username: this.config.username,
          edupage: '' // bei dir offenbar leer, weil du direkt auf rs-kollnau.edupage.org bist
        });
      ...
      // 2) Timetable holen (aktuell: erstmal raw JSON speichern/loggen)
      // -> du hast das Endpoint gefunden:
      //    /timetable/server/currenttt.js?_func=currentttGetData
      const tt = await this.client.getCurrentTimetable();
      this.log.debug(`currentttGetData keys: ${Object.keys(tt || {}).join(', ')}`);

      // 3) TODO: model bauen + writeModel
      // const model = this.convertTimetableToModel(tt);
      ...
      this.setState('info.connection', false, true);
      // wenn Login kaputt, beim nächsten Lauf neu versuchen
      this.isLoggedIn = false;
      throw e;
    }
  }
